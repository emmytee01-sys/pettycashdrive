...
async function seedUserAndLoan() {
    try {
        ...
        await pool.query(
            'INSERT INTO loans (id, loan_reference, user_id, amount, tenure, interest_rate, status, admin_notes, paid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [loanId, ref, userId, 5000000.00, 12, '3.5%', 'disbursed', 'All documents verified and funds transferred.', 0]
        );
